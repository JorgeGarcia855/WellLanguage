...
/*package*/ class WorkersTable_EditorBuilder_a extends AbstractEditorBuilder {
  ...
  public WorkersTable_EditorBuilder_a(@NotNull EditorContext context, @NotNull SNode node) {
    super(context);
    myNode = node;
  }
  ...
  private EditorCell createCollection_0() {
    ...
    editorCell.setCellId("Collection_ll5nah_a");
  ...
  public Grid createChildsVertical_ll5nah_a4a(final EditorContext editorContext, final SNode node) {
    ...
    try {
      ...
      int y = 0;
      Iterable<SNode> elements = SLinkOperations.getChildren(node, LINKS.workers$qH2A);
      for (SNode child : Sequence.fromIterable(elements)) {
        ...
        SubstituteInfo substituteInfo = SubstituteUtil.forChild(editorContext, node, (y < ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.workers$qH2A)).count() ? ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.workers$qH2A)).getElement(y) : null), LINKS.workers$qH2A);
    ...
    if (ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.workers$qH2A)).isEmpty()) {
      EditorCell placeholder = new EditorCellFactory(editorContext, node, grid).createPlaceholderConstant("<no workers>");
      ...
      SubstituteInfo substituteInfo = new DefaultSChildSubstituteInfo(node, null, LINKS.workers$qH2A, editorContext);
  ...
  public HeaderGrid createStaticHeader_ll5nah_e0a4a(final EditorContext editorContext, final SNode snode) {
    ...
    final EditorCell_Constant cell = new EditorCell_Constant(editorContext, snode, "Responsible for Well", false);
    Header header = new EditorCellHeader(new StringHeaderReference("Responsible for Well"), cell);
    header.setLabel("Responsible for Well");
    header.setStyle(style);
    HeaderGrid grid = new HeaderGrid();
    grid.setElement(0, 0, header);
    return grid;
  ...
  public HeaderGrid createHeadQuery_ll5nah_a0e0(final EditorContext editorContext, final SNode node) {
    ...
    IHeaderNodeInsertAction insertAction = new IHeaderNodeInsertAction() {
      public void insertNew(int index) {
        SNodeFactoryOperations.addNewChild(node, LINKS.workers$qH2A, null);
      }
    };
    IHeaderNodeDeleteAction deleteAction = new IHeaderNodeDeleteAction() {
      public void delete(final int index) {
        if (ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.workers$qH2A)).count() > 1) {
          ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.workers$qH2A)).removeElementAt(index);
        }
      }
    };
    HeaderGrid grid = new HeaderGridFactory(editorContext, node, false).createFromObject(queryResult, new StringHeaderReference("8076358177608785090"), insertAction, deleteAction, 0, new ITableStyleFactory() {
      public Style createStyle(final int columnIndex, final int rowIndex) {
        Style style = new StyleImpl();
        final EditorCell editorCell = null;
        return style;
