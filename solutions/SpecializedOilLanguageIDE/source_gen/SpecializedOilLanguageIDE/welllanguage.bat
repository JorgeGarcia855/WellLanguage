...
::----------------------------------------------------------------------
:: WellLanguage startup script.
:: Generated by MPS
::----------------------------------------------------------------------
SET IDEA_PATHS_SELECTOR=WellLanguage1.0
SET PRODUCT=welllanguage
...
IF "%IDE_BIN_DIR:~-8%" == "bin\win\" (
  echo.
  echo    You are trying to run WellLanguage from generic distribution
  echo Please first copy content of "%IDE_BIN_DIR%"
  echo into its parent directory ("%IDE_BIN_DIR:~0,-4%"^)
  echo and then run welllanguage.bat from there
  echo.
  pause
  exit
)
...
:: Try (in order): WellLanguage_JDK, welllanguage%BITS%.exe.jdk, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JDK=
SET IDEA_VENDOR_NAME=null

IF EXIST "%WellLanguage_JDK%" SET "JDK=%WellLanguage_JDK%"
IF EXIST "%JDK%" GOTO check

SET BITS=64
SET USER_JDK64_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\WellLanguage1.0\welllanguage%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\WellLanguage1.0\welllanguage%BITS%.exe.jdk
...
IF NOT EXIST "%JAVA_EXE%" (
  ECHO ERROR: cannot start WellLanguage.
  ECHO No JDK found. Please validate either WellLanguage_JDK, JDK_HOME or JAVA_HOME points to valid JDK installation.
  EXIT /B
)
...
:: ---------------------------------------------------------------------
IF NOT "%WellLanguage_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%WellLanguage_PROPERTIES%"

SET VM_OPTIONS_FILE=
SET USER_VM_OPTIONS_FILE=
IF NOT "%WellLanguage_VM_OPTIONS%" == "" (
  :: 1. %<IDE_NAME>_VM_OPTIONS%
  IF EXIST "%WellLanguage_VM_OPTIONS%" SET "VM_OPTIONS_FILE=%WellLanguage_VM_OPTIONS%"
)
...
SET IDE_JVM_ARGS=-Didea.platform.prefix=Idea -Dintellij.platform.load.app.info.from.resources=true -Didea.jre.check=true -Dpty4j.preferred.native.folder="%IDE_HOME%/lib/pty4j" -Djna.boot.library.path="%IDE_HOME%/lib/jna" -Djava.system.class.loader=com.intellij.util.lang.PathClassLoader
