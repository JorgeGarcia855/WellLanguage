...
# ---------------------------------------------------------------------
# Generated by MPS
IDEA_PATHS_SELECTOR=WellLanguage1.0

message()
{
  TITLE="Cannot start WellLanguage"
...
# Try (in order): $WellLanguage_JDK, .../welllanguage.jdk, .../jbr, .../jbr-x86, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
IDEA_VENDOR_NAME='null'
# shellcheck disable=SC2154
if [ -n "$WellLanguage_JDK" ] && [ -x "$WellLanguage_JDK/bin/java" ]; then
  JDK="$WellLanguage_JDK"
fi

if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/WellLanguage1.0/welllanguage.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/WellLanguage1.0/welllanguage.jdk")
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  ...
  message "No JDK found. Please validate either WellLanguage_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  fi
  exit 1
fi
...
# ---------------------------------------------------------------------
# shellcheck disable=SC2154
if [ -n "$WellLanguage_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$WellLanguage_PROPERTIES"
fi
...
if [ -n "$IDEA_VM_OPTIONS" ] && [ -r "$IDEA_VM_OPTIONS" ]; then
  ...
  VMOPTIONS_FNAME=welllanguage${BITS}.vmoptions
  # 
...
CLASSPATH="$CLASSPATH:$IDE_HOME/lib/mps-scripts-rt.jar"
...
# shellcheck disable=SC2154
if [ -n "$WellLanguage_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$WellLanguage_CLASSPATH"
fi
...
"$JAVA_BIN" \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
  "-XX:ErrorFile=$HOME/java_error_in_WellLanguage_%p.log" \
  "-XX:HeapDumpPath=$HOME/java_error_in_WellLanguage.hprof" \
